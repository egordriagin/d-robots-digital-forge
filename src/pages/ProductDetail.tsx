...
import { fetchProductById } from "@/integrations/supabase/api";
...
const ProductDetail = () => {
  ...
  // --- NEW: Fetch product data using TanStack Query and Supabase ---
  const { data: product, isLoading, isError } = useQuery({
    queryKey: ['product', id],
    queryFn: () => fetchProductById(id!),
    enabled: !!id, // Only run the query if an ID is present in the URL
  });
  ...
  // --- UPDATED: Handle Error or Not Found State ---
  if (isError || !product) {
    return (
      <div className="min-h-screen py-8">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-xl text-gray-600 mb-8">
            Запрашиваемый продукт не существует или произошла ошибка при загрузке.
          </p>
          <Link to="/">
  ...
  // --- The rest of your component now works with live data ---
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
            ...
            <div className="space-y-6">
              ...
              <ProductPricing pricing={product.pricing as {base:string}} shipping={product.shipping as {cost:string, freeThreshold:string}} leadTime={product.leadTime} onRequestOffer={() => setShowOfferForm(true)} onRequestConsultation={() => setShowConsultationForm(true)} />
            </div>
          </div>
